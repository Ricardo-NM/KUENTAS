...
function getPasswordChangeEmailConfig() {
...
function getPasswordChangeEmailCopy(
  language: SupportedLanguage,
  code: string,
  expiresAt: Date,
) {
  const translation = resources[language].translation;
  const copy = translation.email.passwordChange;
...
function buildPasswordChangeEmailHtml({
  appName,
  title,
  intro,
  codeLabel,
...
export async function sendPasswordChangeCodeEmail({
  ...
  const config = getPasswordChangeEmailConfig();
  const copy = getPasswordChangeEmailCopy(
  ...
  await transporter.sendMail({
    ...
    html: buildPasswordChangeEmailHtml({
      ...copy,
      code,
    }),
  });
