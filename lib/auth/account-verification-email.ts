...
function getAccountVerificationEmailConfig() {
...
function getAccountVerificationEmailCopy(
  language: SupportedLanguage,
  code: string,
  expiresAt: Date,
) {
  const translation = resources[language].translation;
  const copy = translation.email.accountVerification;
...
function buildAccountVerificationEmailHtml({
  appName,
  title,
  intro,
  codeLabel,
        ...
        <tr>
          <td style="border:1px solid ${emailColors.border};border-radius:16px;background:${emailColors.surface};padding:28px;">
            ...
            <div style="margin:0 0 20px 0;border:1px solid ${emailColors.border};border-radius:12px;background:${emailColors.background};padding:20px;text-align:center;">
              ...
              <div style="font-family:'JetBrains Mono','SFMono-Regular',Consolas,monospace;font-size:32px;line-height:40px;font-weight:700;letter-spacing:0.16em;color:${emailColors.primary};">
                ${code}
              </div>
            </div>
            ...
            <div style="margin-top:20px;height:3px;width:48px;border-radius:999px;background:${emailColors.success};"></div>
          </td>
        </tr>
      </table>
    </div>
...
export async function sendAccountVerificationCodeEmail({
  to,
  code,
  expiresAt,
  language,
  ...
  const config = getAccountVerificationEmailConfig();
  const copy = getAccountVerificationEmailCopy(
